...
def quarter_chart_line_avg(data_avg,quarter_num,custom_palette,strokeDashOption = [1,0]):
    team_data_qi = data_avg[data_avg['qtr']==quarter_num]
    if strokeDashOption != [1,0]:
        team_data_qi = team_data_qi[team_data_qi['made_shot']==True]
    # st.write(team_data_qi[['qtr','simple_timestamp','inside']].head())
    # st.write(values)
    team_data_qi['Minute'] = team_data_qi['minute']+1
    opacity_scale = alt.Scale(domain=[True,False],range=[0.5,0.3] )
    chart_qi = alt.Chart(team_data_qi).mark_line(    strokeDash=strokeDashOption).encode(
    x=alt.X('simple_timestamp:Q',axis = alt.Axis(labelColor='gray',titleColor = 'black',labels =False,ticks = False,grid = False,domain = True,domainCap='round',domainColor='lightgray',title='',titleFontWeight=100,)),
    # x=alt.X('simple_timestamp:Q',axis = alt.Axis(labelColor='gray',titleColor = 'black',labels =False,ticks = False,grid = False,domain = False,domainCap='round',domainColor='dimgray',title='Q'+str(quarter_num),titleFontWeight=100,)),
    y=alt.Y('sum(count):Q',axis = alt.Axis(labelPadding=5,labelColor='black',labels =False,titleColor = 'gray',ticks = False,grid = False,domain = False,domainCap='round',titleFontWeight=100,title = None,titleAngle=0,titlePadding=40 )),
...
def timeline_chart_avg(team_data,custom_palette,WIDTH,HEIGHT,values,data):
    ...
    quarter_total_height = HEIGHT* 0.65 * 0.6 *0.5
    team_data_agg = team_data.groupby(by=['simple_timestamp','shooting_team','qtr','made_shot']).size().reset_index().rename(columns={0:'count'})
    data_agg = data.groupby(by=['simple_timestamp','shooting_team','qtr','made_shot']).size().reset_index().rename(columns={0:'count'})
    ...
    data_agg['minute'] = (data_agg['qtr']-1)*10 + team_data_agg['simple_timestamp']//60
    data_agg['inside'] = data_agg.apply(lambda row: is_inside(row,values),axis = 1)
    data_agg_avg = data_agg.groupby(by = ['simple_timestamp','minute','qtr','made_shot'])['count'].mean().reset_index()
    ...
    q1 = (y_axis_reference(team_data_agg[team_data_agg['qtr']==1],limit)+quarter_chart_line_avg(data_agg_avg,1,custom_palette)+quarter_chart_line(team_data_agg,1,custom_palette)).resolve_scale(opacity = 'independent').properties(width=quarter_chart_width,height = quarter_chart_height)
    q2 = (y_axis_reference(team_data_agg[team_data_agg['qtr']==2],limit)+quarter_chart_line_avg(data_agg_avg,2,custom_palette)+quarter_chart_line(team_data_agg,2,custom_palette)).resolve_scale(opacity = 'independent').properties(width=quarter_chart_width,height = quarter_chart_height)
    q3 = (y_axis_reference(team_data_agg[team_data_agg['qtr']==3],limit)+quarter_chart_line_avg(data_agg_avg,3,custom_palette)+quarter_chart_line(team_data_agg,3,custom_palette)).resolve_scale(opacity = 'independent').properties(width=quarter_chart_width,height = quarter_chart_height)
    q4 = (y_axis_reference(team_data_agg[team_data_agg['qtr']==4],limit)+quarter_chart_line_avg(data_agg_avg,4,custom_palette)+quarter_chart_line(team_data_agg,4,custom_palette)).resolve_scale(opacity = 'independent').properties(width=quarter_chart_width,height = quarter_chart_height)
